...
class SVsTimingProducer : public edm::stream::EDProducer<> {
public:
  explicit SVsTimingProducer(const edm::ParameterSet&);
  ~SVsTimingProducer() override;
...
SVsTimingProducer::SVsTimingProducer(const edm::ParameterSet& iConfig) {

  transientTrackBuilder_ = esConsumes(edm::ESInputTag("", "TransientTrackBuilder"));

  gsfElectronToken_ = consumes<reco::GsfElectronCollection>(iConfig.getParameter<edm::InputTag>("electronSrc"));
...
SVsTimingProducer::~SVsTimingProducer() {}
...
void SVsTimingProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  using namespace edm;
  ...
  const reco::GsfElectronCollection* electronCollection = gsfElectrons.product();
  ...
  // Create container for storing transient tracks (converted from gsfTracks from input gsfElectrons)
  ...
  if(electronCollection->size() < 2) {
    iEvent.put(std::move(timedVertices), vertexLabel_);
    return;
  }
  ...
  std::map<std::pair<double, double>, const reco::GsfElectron> trackToElectronMap;
  ...
  for (eleIt = electronCollection->begin(); eleIt != electronCollection->end(); eleIt++){
    const reco::GsfElectron electron = *eleIt;
    const reco::GsfTrackRef gsfTrack = electron.gsfTrack();

    if(gsfTrack.isNull()) 
      continue;

    // Convert the electron gsfTrack to a transient track (needed by the AVR algorithm)
    const reco::TransientTrack track = ttBuilder->build(gsfTrack);
  ...
  for(auto const &vertex : vertexCollection) {
    ...
    for(auto const &track : vertex.originalTracks()) {
      ...
      const reco::GsfElectron trackElectron = trackToElectronMap[std::make_pair(recoTrack.chi2(), recoTrack.pt())];
      const math::XYZPointF trackDestination = trackElectron.trackPositionAtCalo();
...
reco::Vertex SVsTimingProducer::ConvertFitVertex(const TransientVertex &vertex) const {
...
reco::Track SVsTimingProducer::ConvertTrack(const reco::Track &track, const double t0, const double beta) const {
...
double SVsTimingProducer::LinearDistance(const GlobalPoint &initialPosition, const GlobalPoint &destination) const {
...
double SVsTimingProducer::PathLength(const reco::TransientTrack &track, const GlobalPoint &vertexPosition, const GlobalPoint &positionAtECAL) const { 
...
void SVsTimingProducer::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  edm::ParameterSetDescription desc;
  desc.add<edm::InputTag>("electronSrc", edm::InputTag("lowPtGsfElectrons"));
  desc.add<edm::InputTag>("timedVertices", edm::InputTag("timedSecondaryVertices"));
  desc.add<std::string>("vertexLabel", std::string("vertexLabel"));
  descriptions.addDefault(desc);
}

//define this as a plug-in
DEFINE_FWK_MODULE(SVsTimingProducer);
