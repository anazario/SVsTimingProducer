#ifndef TimingWithSVs_SVsTimingProducer_DeltaRMatch_h_
#define TimingWithSVs_SVsTimingProducer_DeltaRMatch_h_
...
template <class A, class B> class DeltaRMatchHungarian {
 ...
 private:
  ...
  std::vector<MatchedPair> ConstructMatchedPairs(const std::vector<std::vector<double> > &costMatrix, const std::vector<A> &objectsA, const std::vector<B> &objectsB) {

    matchedObjectsA_.clear();
    matchedObjectsB_.clear();

    //std::cout<< "There are "<< objectsA.size() << " objects in A and " << objectsB.size() << " objects in B " << std::endl;
    std::vector<MatchedPair> matchedPairs;
    for(unsigned int mi = 0; mi < costMatrix.size(); mi++) { 
      if(matchedIndexes_[mi] == -1)
	continue;

      //std::cout<< "I think index A is " << mi << " and index B is " <<matchedIndexes_[mi] << std::endl;      
      //std::cout << "matched index: " << matchedIndexes_[mi] << ", at row: " << mi << std::endl;
      
      const double deltaR = costMatrix[mi][matchedIndexes_[mi]];
      const MatchedPair thisMatchedPair(mi, matchedIndexes_[mi], deltaR);

      //std::cout<< "with deltaR: " << deltaR << std::endl;
      ...
      //std::cout<< "Let's get that deltaR again to be sure: " << pairedObjects_[mi].GetDeltaR() << std::endl;
    }
